...
- **Phase 2(E4: model-gateway + compose統合)**: **完了**。E4-1〜E4-5
  (#37, #83, #84, #85, #86)、親issue #4 全てクローズ済み。
  `docker compose --profile api-only up -d` で常時稼働9サービス
  (web-ui/api-server/agent-core/pmdf-store/model-gateway/qdrant/minio/
  mlflow/scheduler)が全て `healthy` になることを AC-01 で検証済み。
  バックエンド無改修切替は `.env` の `ACTIVE_BACKEND=api|ollama` と
  `scripts/switch_backend_check.sh` で AC-02 検証済み。起動手順は
  本ファイル末尾の「model-gateway 公開インターフェース」を参照。
...
- **Phase 3(E7: web-ui)**: **完了**。E7-1〜E7-9(#51 be34c88 / #52
  bfab912 / #53 32bdbe3 / #54 8b052b0 / #55 d20a781 / #56 978dc63 /
  #57 1e07827 / #58 63329bc / #59)。web-ui骨格(認証・APIクライアント
  (`openapi-typescript`生成型)・レイアウト・WebSocketクライアント・承認
  バッジ・プロダクトダッシュボード)に加え、承認キュー・ドキュメント
  ビューア・エージェント活動ログ・手動編集(EntityEditor/YamlJsonEditor)・
  Import/Export(競合解決)・エージェント制御(自律レベル/チャット指示/
  緊急停止)・コスト/学習状況画面を実装済み。起動方法は
...
- **Phase 4(E8: 自己学習ループ)**: **完了**。E8-1〜E8-8(#60〜#67)、
  親issue #8 全てクローズ済み。`packages/pmdf/learning/`(スキーマ・split・
  contamination)・`agent_core/learning/`(合成・実行・評価・データセット化)・
  `services/trainer`(QLoRA SFT/DPO)・`services/eval-runner`(300問ベンチ・
  昇格ゲート)を実装。公開インターフェースは本ファイル末尾の
  「自己学習ループ公開インターフェース(E9以降が利用する想定)」を参照。
  web-ui の学習進捗は `eval-runner compare` 結果または MLflow UI へ接続予定
  (E7-9 プレースホルダ)。
- **Phase 5(E9: 運用機能)**: **完了**。E9-1〜E9-5(#68〜#72)、
  親issue #9 全てクローズ済み。`services/scheduler`(KPI監視・週次レビュー・
  学習ループ cron、`budget_monitor` による予算ポーリングと学習ブロック)、
  `docs/logging-conventions.md`(structlog 規約)、`scripts/backup.sh`/
  `restore.sh`(`.env` 除外)、`.github/workflows/security-scan.yml`
  (Trivy/pip-audit/pnpm audit + pinact/zizmor)を実装。
- **Phase 5(E10: E2E受け入れとドキュメント)**: **完了**。E10-1〜E10-4
  (#73〜#76)、親issue #10 全てクローズ済み。`scripts/seed_sample_product/`、
  Playwright E2E(`full_pdm_cycle`/`approval_bypass_ui`/`bundle_roundtrip`/
  `bundle_sanitize`)、運用ドキュメント4種(`setup-tier-l`/`operations-policy`/
  `license-ledger`/`local-verification-checklist`)を整備。
- **アクティブブランチ: `main`**。Phase 5 完了後は `main` がリリース基準。
  新規実装は `main` から feature ブランチを切り PR マージする。

## 再開手順

1. `git switch main && git pull` でローカルを最新化する。
...
| E4       | #4        | model-gateway + compose統合   | phase:2    | **完了**(E4-1〜E4-5 = #37,#83,#84,#85,#86)            |
| E5       | #5        | agent-core                    | phase:3    | **完了**(E5-1〜E5-9 = #38〜#46)                       |
| E6       | #6        | 知識ベース                    | phase:3    | **完了**(E6-1〜E6-4 = #47〜#50)                       |
| E7       | #7        | web-ui                        | phase:3    | **完了**(E7-1〜9=#51,#52,#53,#54,#55,#56,#57,#58,#59) |
| E8       | #8        | 自己学習ループ                | phase:4    | **完了**(E8-1〜E8-8 = #60〜#67)                       |
| E9       | #9        | 運用機能                      | phase:5    | **完了**(E9-1〜E9-5 = #68〜#72)                       |
| E10      | #10       | E2E受け入れとドキュメント     | phase:5    | **完了**(E10-1〜E10-4 = #73〜#76)                     |
...
`services/model-gateway`(E4-1〜E4-5で実装)は、全LLM・埋め込み呼び出しの
単一窓口となるLiteLLM Proxy設定(`litellm.config.yaml`)を提供する。
E4-4で `docker compose --profile api-only` によるコンテナ統合起動が
可能。E5(agent-core)以降は下記を前提にクライアントを実装すること。

### compose統合起動(E4-4、AC-01)

```bash
cp .env.example .env   # 初回のみ
docker compose --profile api-only up -d
docker compose --profile api-only ps   # 9サービスすべて healthy を確認
docker compose --profile api-only down
```

CIでは実コンテナ起動は行わず、`.github/workflows/ci-compose.yml` で
`config` + `build` を検証する。静的テストは
`scripts/tests/test_compose_healthcheck_definitions.py`。

### バックエンド無改修切替(E4-5、AC-02)

`.env` の `ACTIVE_BACKEND=api|ollama` のみで `pdm-main` の実バックエンドを
切替(`services/model-gateway/docker-entrypoint.sh` が
`PDM_MAIN_MODEL`/`PDM_MAIN_API_BASE`/`PDM_MAIN_API_KEY` を設定。
`litellm.config.yaml` と `docker-compose.yml` は変更しない)。

```bash
bash scripts/switch_backend_check.sh
```

Ollama 実環境が無い場合は OpenAI 互換モック
(`services/model-gateway/tests/mock_openai_compatible_server.py`) で代替検証する。
...
- `pdm-student`: 自己学習ループでSFT/DPOしたアダプタの評価対象role
  (E8のeval-runner比較で`pdm-teacher`と対比するstudent側、
  `services/model-gateway/litellm.config.yaml`に定義済み)。
...
## 自己学習ループ公開インターフェース(E9以降が利用する想定)

### agent-core learning (`agent_core.learning`)

- `synthesize_scenarios(kb_chunks, pmdf_samples, count, llm_client)` → `list[Scenario]`
- `execute_scenario(scenario, llm_client, sandbox_store, production_store)` → `TrajectoryRecord`
- `hybrid_evaluate(trajectory, llm_client)` → `EvaluationResult`
- `collect_preference_pairs_from_queue(queue_dir)` → `list[DpoRecord]`
- `build_sft_dataset` / `build_dpo_dataset` / `filter_by_opt_in` / `OptInStore`

### trainer (`services/trainer`)

- `run_sft(dataset_path, TrainingConfig)` / `run_dpo(...)` — TRL + LoRA、MLflow(sqlite)
- `bnb_adapter.load_model_4bit` — 関数境界モック可能( CI/GPU 非搭載向け)
- CLI: `uv run trainer-sft data.jsonl` / `uv run trainer-dpo data.jsonl`

### eval-runner (`services/eval-runner`)

- `load_bench_dir(bench/)` — 300問(6カテゴリ×50) YAML 検証
- `compare_models(baseline_scores, new_scores)` — PdM +10pt かつ一般能力 -5pt 以内で promote
- `promote_model(adapter_path, ollama_url)` — Ollama アダプタ差替(HTTP)
- CLI: `uv run eval-runner run-bench|compare|promote`
...
| 2026-07-05 | E7全9サブイシュー(#51〜#59)完了。E7-9では学習進捗をE8 API未実装のためモック表示とし、Playwrightで768px/1280pxのレスポンシブE2Eを`ci-web.yml`に追加した                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                  | FR-UI-10の受入条件(Playwrightビューポート検証)とFR-UI-08(E8 API未着手時のプレースホルダ許容)を満たすため                                                                                                                                                                                                                              |
| 2026-07-05 | E4-4(#85)完了: 常時稼働9サービスのDockerfile・healthcheck・`docker compose --profile api-only` 統合起動(AC-01)。E4-5(#86)完了: `ACTIVE_BACKEND` による `.env` のみでのバックエンド切替と `scripts/switch_backend_check.sh`(AC-02)。親issue #4 クローズ                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                  | PR #97/#98 で実装済み。ローカルで compose up(全healthy)・switch_backend_check・pytest 485 passed で受け入れ条件を再検証した                                                                                                                                                                                                           |
| 2026-07-05 | E8全8サブイシュー(#60〜#67)完了、親issue #8クローズ。自己学習ループ公開インターフェースを本ファイルに追記                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                               | E9(scheduler)が `trigger_learning_loop()` で連結する想定。trainer/eval-runner は `train` compose profile                                                                                                                                                                                                                              |
| 2026-07-06 | E9全5サブイシュー(#68〜#72)完了、親issue #9クローズ。scheduler ジョブ・予算監視・ログ規約・バックアップ/リストア・脆弱性スキャン CI を実装                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                              | Phase 5 運用機能。`docs/logging-conventions.md`、`.github/workflows/security-scan.yml`                                                                                                                                                                                                                                                |
| 2026-07-06 | E10全4サブイシュー(#73〜#76)完了、親issue #10クローズ。サンプルシード・Playwright E2E(AC-05〜08)・運用ドキュメント4種を整備し **Phase 5 完了**                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                          | `docs/{setup-tier-l,operations-policy,license-ledger,local-verification-checklist}.md`。GPU 系 AC-03/04/10 は人手チェックリスト                                                                                                                                                                                                       |
| 2026-07-08 | Phase 5完了後の実装監査で見つかった当初計画との食い違い6件(`fix/plan-gap-reconciliation`)を解消。(1) trainer/eval-runnerを`sleep infinity`プレースホルダから`services/scheduler/Dockerfile`踏襲のuvベース実コンテナへ変更(`build:`化、`docker compose --profile train build`で検証)。(2) `services/eval-runner/bench/*.yaml`のダミー300問(定型文言い回しのみ)を、カテゴリ毎に趣旨に沿った実質的な問題文・問題固有の期待基準に置き換え(`scripts/generate_bench_questions.py`は初期スキャフォールド専用と明記)。(3) trainer(sft/dpo)・eval-runner(compare)の完了時に`LEARNING_STATUS_PATH`(共有ボリューム`learning-status`)へJSONLでステータスを追記し、api-serverに`GET /learning/status`を新設、`CostAndLearning.tsx`の`learning-placeholder`モックを実データ表示へ置換。(4) `agent_core.runner`(新設)がapi-serverの`GET /chat/tasks?status=pending`をポーリングし`execute_chat_task`(`handle_chat_instruction`から抽出)を実行、緊急停止・学習ブロックを毎サイクル確認、失敗時は`failed`遷移しループ継続。`__main__.py`に統合(`RUNNER_ENABLED`/`RUNNER_POLL_INTERVAL_SEC`)。(5) `litellm.config.yaml`冒頭コメントと本ファイルの論理名一覧に`pdm-student`を追記。(6) Dependabot alert #1(critical、`aquasecurity/trivy-action`)をv0.35.0へ更新して解消。 | 実装済みコードと計画ドキュメント・compose定義の乖離を1本のPRで解消し、以降のセッションが本ファイルの記述を信頼して再開できるようにするため。常駐ランナーの4業務グラフへの実アダプタ配線(intake_text等シグネチャ差異の吸収)は本PRのスコープ外として残課題化した(`agent_core.runner._default_dispatch`は既存同様のプレースホルダのまま) |
